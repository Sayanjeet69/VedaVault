...
consumption without adding interpretation. A future LLM will synthesize an
answer from that context. The boundary is deliberate: scripture is retrieved
evidence; interpretation and modern application are not generated by either
`EvidenceBundle` or `GroundingContext`. Neither an LLM nor an application layer
