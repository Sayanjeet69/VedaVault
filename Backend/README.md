...
application are separate fields. The future LLM will eventually generate those
fields; the contract and validation remain deterministic. The boundary is
deliberate: scripture is retrieved evidence; interpretation and modern
application are not generated by `EvidenceBundle` or `GroundingContext`.
Neither an LLM nor an application layer is part of this package yet.
